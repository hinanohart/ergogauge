...
stationary block-bootstrap). The shipped numbers are generated with `bootstrap_reps=30`
(the CI-gated label clears its threshold by a wide margin in this regime, so it is robust to
reps; fewer reps only widen CIs, the fail-closed direction — `CLAIM.md` §4 default is 1000).
