...
All numbers below are generated by `scripts/gen_metrics.py` into [`results/0.1.0a2_metrics.json`](results/0.1.0a2_metrics.json) on the **full 100-seed held-out TEST set** (seeds 1000–1099); thresholds were calibrated on the disjoint TRAIN seeds (0–99). Regime: easy-extreme (V=64, L=2000, 16 utterances/corpus, stationary block-bootstrap).
...
**Honest scope — thresholds are calibrated for V=64.** A grid-robustness sweep shows the pre-registered thresholds do **not** yet transfer across alphabet sizes: at V=16 the HEALTHY generator is mis-flagged and at V=256 OVER_RANDOM / REPETITIVE are mis-flagged. Per-alphabet calibration is **ROADMAP**; v0.1.0a2 claims the easy-extreme V=64 regime only. Reproduce: `python scripts/gen_metrics.py`.
