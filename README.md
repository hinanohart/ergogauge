...
> **What it is / is not**
>
> - **Is:** a CPU-only, reference-free, decode-free, LLM-free diagnostic instrument that treats a generated codec-LM token stream as a finite-state Markov chain and reports an ergodicity certificate (spectral gap, Kemeny constant, Cheeger/Fiedler near-reducibility) from the empirical token-transition operator.
> - **Is NOT:** a replacement for FAD/MMD/UTMOS (complementary, reference-free, decode-free axis); not an audio-quality benchmark; makes no claim about perceptual quality or that it fixes/solves generation quality.
> - **Heuristic caveat:** The invariant→failure-mode mapping on non-reversible chains is a calibrated heuristic, not a theorem. Flags are reported with bootstrap confidence intervals and fail-closed to `ABSTAIN` below the pre-registered identifiability threshold.
> - **Correctness basis:** Primary correctness is demonstrated on a synthetic injected-pathology harness; any real codec-LM token demo is secondary and labelled synthetic-vs-real.
...
All numbers below are generated by `scripts/gen_metrics.py` into [`results/0.1.0a2_metrics.json`](results/0.1.0a2_metrics.json).

- **Test set:** full 100-seed held-out TEST set (seeds 1000–1099); thresholds calibrated on disjoint TRAIN seeds (0–99).
- **Regime:** easy-extreme (V=64, L=2000, 16 utterances/corpus, stationary block-bootstrap).
- **Bootstrap reps:** shipped numbers use `bootstrap_reps=30`; the CI-gated label clears its threshold by a wide margin in this regime (fewer reps only widen CIs in the fail-closed direction). `CLAIM.md` §4 default is 1000.
...
- Severity tracked monotonically: Spearman of `(ρ, −spectral_gap)` for REPETITIVE = **1.00** and of `(−log₁₀ δ, −cheeger_phi)` for LOCKED = **1.00**.
...
- Order-sensitivity sanity check: ergogauge spectral gap separates a loop from its shuffle (AUROC **1.00**) where the order-independent in-repo Vendi cannot (AUROC **0.50**). Structural check, **not** a superiority claim (`vendi_wedge=false`).

**Honest scope — thresholds are calibrated for V=64.**

A grid-robustness sweep shows the pre-registered thresholds do **not** yet transfer across alphabet sizes:

- V=16: HEALTHY generator is mis-flagged (recall 0.0).
- V=256: OVER_RANDOM / REPETITIVE are mis-flagged.

Per-alphabet calibration is **ROADMAP**; v0.1.0a2 claims the easy-extreme V=64 regime only. The easy-extreme numbers are saturated by construction; harder regimes (weak loops, sub-resolution locks, single short utterances) fail **closed to ABSTAIN** rather than guessing.

Reproduce: `python scripts/gen_metrics.py`.
