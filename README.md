...
[`results/0.1.0a1_metrics.json`](results/0.1.0a1_metrics.json) and measured on the
**held-out TEST seed grid** (seeds 1000–1002); thresholds were calibrated only on the
disjoint TRAIN seeds (0–99). Regime: easy-extreme (V=64, L=2000, 16 utterances/corpus,
stationary block-bootstrap, 60 reps).

| ground truth | detected | recall |
|---|---|---|
...
- macro recall **1.00**; HEALTHY false-flag rate **0.00**.
- degenerate inputs (short, underdetermined) ABSTAIN at rate **1.00** (fail-closed).
- order-sensitivity sanity check: the ergogauge spectral gap separates a loop from its
  ...
  **0.50**). This is a structural check, **not** a superiority claim (`vendi_wedge=false`;
  ergogauge and Vendi are complementary axes).

These easy-extreme numbers are saturated by construction; the harder regimes (weak loops,
near-threshold cross-probabilities, single short utterances) are where ergogauge fails
closed to ABSTAIN rather than guessing. Reproduce: `python scripts/gen_metrics.py`.
